...
\begin{equation} \label{eq:sch_indep_linear}
...
\begin{equation} \label{eq:lin_hom}
...
\begin{equation} \label{eq:sol_gen_2}
...
\begin{equation} \label{eq:sol_gen_3}
...
\begin{equation} \label{eq:sol_gen_4}
...
\begin{equation} \label{eq:psi_solution}
\psi = \sqrt{\frac{2}{L}} \sin \left( \frac{n \pi x}{L} \right ), ~~~~~~~ n=1,2,3...
\end{equation}
<h2>Particle in a spherically symmetric infinite potential well</h2>
...
\begin{equation} \label{eq:pot_sph}
U = U(r)
\end{equation}
...
\begin{equation} \label{eq:psi_sphere}
...
\begin{equation} \label{eq:psi_sphere_2}
...
\begin{equation} \label{eq:psi_sphere_3}
...
\begin{equation} \label{eq:psi_sphere_final}
...
\begin{equation} \label{eq:ps_energy}
...
\begin{equation} \label{eq:ferrel2}
E = E_{\rm Ps} + E_{\rm surface} \to \text{minimize over}~~ R_\infty
\end{equation}
...
\begin{equation} \label{eq:tao_psi}
...
\begin{equation} \label{eq:tao_rate}
...
\begin{equation} \label{eq:tao_delta}
...
<div class="row">
    <div class="col-sm-6">
        ...
        <small>Dependence of the pick-off annihilation rates vs. surface tension in different liquids. Solid curve shows the correlation given by the Tao-Eldrup at λ+ = 2 ns−1 and optimal value $\delta=1.66$ Å (obtained from fitting of this data by means of Eq. \eqref{eq:tao_delta}). Dashed curve illustrates simplest approximation $\lambda_{po} \propto \sigma^{1/2}$.</small>
    </div>
    <div class="col-sm-6">
        Substituting \eqref{eq:bubble_radius} for $R_\infty$ into \eqref{eq:tao_delta}, we obtain the relationship between $\lambda_{po}$ and $\sigma$ with one adjustable parameter, $\delta$. It may be easily obtained by fitting experimental pick-off annihilation rate constants with the relationship \eqref{eq:tao_delta}, see figure. From the fit we obtain $\delta = 1.66$ Å.
        Equation \eqref{eq:tao_R} with this value of $\delta$ is widely used for recalculation of the observed pick-off annihilation rate into the free volume $4\pi R^3/3$ of the cavity, where Ps atom resides and annihilates.:
        \begin{equation}
